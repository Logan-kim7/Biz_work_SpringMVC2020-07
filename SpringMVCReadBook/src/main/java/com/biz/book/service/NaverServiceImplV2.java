...
public String queryURL(String category, String bookName) {
	...
	// NaverServiceV1의 3개의 method 를 모두 상속받아서
	// 여기에 아무런 코드가 없어도 프로젝트가  정상 실행이 된다.
	...
	public List<BookVO> getNaverList(String queryURL) {
		...
		// quertURL 문자열을 URL 객체가 아닌 URI 객체로 만들어야 한다.
		URI restURI = null;
		 ...
		 * 복잡하게 만들어야 했다.
		 * 
		 * spring에서 외부 API를 사용하여 데이터를 가져올일이 점점 많이 지면서
		 * RestTempate라는 클래스를 새로 만ㄷ르어
