 ...
 * authentication-manager-provider 클래스 Customizing 수행하기 
 */
@Slf4j
public class AuthProviderImpl implements AuthenticationProvider {
	// spring security를 통하여 login 을 수행헀을때 호출되는 method
	@Override
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		if( !bUser) {
			 ...
			 * 어떤문제가 발생을 하면
			 * 메세지를 만들고 강제로 exception을 발생시키면
			 * spring security에게 메세지를 전달하는 효과가 나타난다.
			 * 
			 * Authenticate() method는 실행을 멈추고
			 * spring security 가 메세지를 수신하여 다시 login 하면을 열고
			 * 메시지를 보여준다. 
			 */
		...
		if(!userVO.isAccountNonLocked()) {
			throw new LockedException("사용자계정이 잠겨있습니다.<br>관리자에게 문의 하세요");
...
//		
		...
		// 사용자 데이터를 생성하고 login 성공 메시지를 만들기
	...
	// 현재 만들어지 authProviderimpl을 spring seucrity에서 사용가능하도록 설정
