...
<%/*
	...
	권한에 따라서  메뉴를 보이고 보이지 않도록 하는 용도록 사용하는 taglib
	...
	<ul>
		...
		*/%>
		<sec:authorize access="isAnonymous()">
			...
			<%/*
			isAuthenticated() 현재 화면에서 권한에 관계없이 로그인한 세션정보가 있으면
			mypage, 로그아웃 메뉴를 보여라
	     */%>
		...
		<li>로그아웃</li>			
			...
			<%/* hasRole('ROLE_ADMIN') 로그인 세션정보가 있고, ROLE(권한정보가)  ADMIN 이면 */%>
		<sec:authorize access="hasRole('ROLE_ADMIN')">
			<li>관리자</li>
		</sec:authorize>
		
		<%/* hasRole(ROLE_USER) 로그인 세션정보가 있고, ROLE(권한정보가)  USER 이면  */%>
		<sec:authorize access="hasRole('USER')">
			<li>일반 사용자</li>
		</sec:authorize>
		
		<%/* hasRole(ROLE_GUEST) 로그인 세션정보가 있고, ROLE(권한정보가)   GUEST 이면 */%>
