...
public class BBsAPIController {
	...
	// http://127.0.0.1:5500 에서 api 요청이 오면, CORS Policy를 무시하고 응답하라
	// @CrossOrigin("http://127.0.0.1:5500")
	// localhost:8080/bbs/api/bbs 요청을하면 게시판 list를 보내달라(getter 하겠다)
	...
	// localhost:8080/bbs/api/bbs/3게시판 데이터중에서 3번데이터 1개를 보내달라
	...
	// 보내는 방식에 따라서 문자열형 변수로 보낼수도 있고
	// Json형식으로 보낼수도 있다.

	// 문자열 형식으로 보내면 String id 와 같이 받으면 되는데
	// JSON형식으로 보냈을 때는 약간의 문제를 일으킨다.
	// 이유는 Spring이 태생적으로 JOSON을 지원하지 않는다.

	// 그래서 사용하는 방법이 Map<String, String> 형식의 매개변수를 만들어주고
	// @RequestBody를 붙여주는 방식을 사용한다.
	// @RequestBody는 절때 생략할 수 없다.
