...
public class UserDetailServiceImplV1 implements UserDetailsService {
	 ...
	 * 
	 * 주입받을 객체를 final로 선언을 해주는데 final로 선언한 객체는 반드시 생성자에서 객체 초기화(주입)을 해야한다.
	 * 
	 * 1. 주입받을 객체를 final로 선언하고 2. 생성자의 매개변수를 통일하여 객체를 초기화 한다. 3. 주입받을 객체의 개수가 늘거나 줄면
	 * 생성자를 또다시 변경해야하는 번거로움이 있다.
	 * 
	 * 
	 * lombok의 @RequiredArgsConstructor 를사용하면 final로 선언된 모든 필드변수들을 모아서 생성자로 만들어준다.
	 */
...
//	} 
	...
	/*
	 * 이프로젝트에서 사용할 memver(user)관련 table에서 username으로 사용자 정보를 SELECT 하고 사용자의 ROLL 정보를
	 * 기준으로 사용자의 권한을 설정하여 기능을 수행을 제한하는 설정을 하고 사용자의 여러 세부 정보를 VO객체에 담아주는 역할 수행
	 */
