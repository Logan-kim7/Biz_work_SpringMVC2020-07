 ...
 * 회원가입 정책(Policy) *
 * 1. tbl_member table 에 아무런 데이터도 없이 최초로 가입(INSERT)된 
 ...
 * 
 * 4. 같은아이디로  재 가입을 요청하면 권한을 GUEST에서 USER로 변경한다.
 * 
 ...
 */
	...
	public int insert(MemberVO memVO) {
		...
		/*
		 * if(count == 0){
		 * 		memVO.serM_roll("ADIN")
		 * }
		 * 
		 * 테이블에 데이터가 하나도 없을때는 ADMIN으로 설정한다는
		 * 이 비교연산식은 문법적으로나 논리적으로 아무런 문제가 없는 코드이다.
		 * 
		 * 하지만, 실제상황에서 JDBC를 통해서 서버에 원격연결을 하는 상황에서는
		 * Count값이 실제로는 0이지만 0이 아닌 -1등의 값이 될수도 있다.
		 * 데이터가 없을때 0dlrjsk -1인 경우가 된다면 
		 * if(count == 0 || count == -1) 또는
		 * if(count <= 0)과 같은 비교연산식이 만들어 져야한다.
		 * 비교연산식에서 가장 권장하는 (좋은0 코드는
		 * 가급적 비교대상을 부등호로 한번만 비교하는 것이다.
		 * else 가 있는 상황이므로 
		 * 더 값을 근접하게 찾는 연산식으로 변경을 하는것이 좋다.
		 * 
		 * if(count >0) => 이상적인 코딩 
		 * 
		 * 
	 ...
	 * 로그인 절차
	 * 
	 * 1. loginVO에 담긴 정보에서 userid를 추출하여 사용자 정보를 가져오기 findByID()
